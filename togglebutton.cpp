...
void ToggleButton::paintEvent(QPaintEvent *)
{
    ...
    p.drawRoundedRect(mMargin, 0.25 * height(), width() - 2 * mMargin, 1.5 * mTrackRadius, mTrackRadius, mTrackRadius);
...
void ToggleButton::updateOffsetRange()
{
    // 水平移动区间：从 (左边缘 + thumbRadius) 到 (右边缘 - thumbRadius)
    qreal start = mMargin + mThumbRadius;
    qreal end   = width() - mMargin - mThumbRadius;
    mEndOffset[false] = start;
    mEndOffset[true]  = end;
}


ToggleButton::ToggleButton(QWidget *parent)
: ToggleButton(10, 12, parent)
{}
